import { getCurrentInstance, version, reactive, h, isReadonly, defineComponent, computed, ref, resolveComponent, createApp, toRef, isRef, defineAsyncComponent, provide, onErrorCaptured, onServerPrefetch, unref, useSSRContext, inject } from 'vue';
...
import { isEqual, hasProtocol, parseURL, joinURL, stringifyParsedURL, stringifyQuery, parseQuery } from 'ufo';
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    const { data: temp } = useFetch("https://httpbin.org/get", "$E3KxD8SGyR");
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(_attrs)}><h1>Hello from Nuxt</h1><div>${ssrInterpolate(unref(temp))}</div></div>`);
    };
  }
};
...
const _sfc_main = {
  ...
  __ssrInlineRender: true,
  setup(__props) {
    const ErrorComponent = defineAsyncComponent(() => import('./error-component-d2f06986.mjs').then((r) => r.default || r));
    const IslandRendererer = defineAsyncComponent(() => import('./island-renderer-e10ae991.mjs').then((r) => r.default || r));
    const nuxtApp = useNuxtApp();
...
export { __nuxt_component_0 as _, createError as c, entry$1 as default, useNuxtApp as u };
